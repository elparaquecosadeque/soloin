...
- **Show the user's marks (read-only) on the Progression fretboards too.** Marks are made and shown only in Key mode (`keyMarks`, saved with the-chords' "Save session" via `getSessionState()`/`applySessionState()`). In Progression (one board per chord) they don't appear yet; drawing them there, read-only, would echo the song-level marks over each chord's tones. Not done on purpose: it needs a decision on clutter in Mosaic view.
- **Mention Identify and marks in the-chords' info modal / tour.** Neither was updated when Identify and the mark-notes feature shipped.
